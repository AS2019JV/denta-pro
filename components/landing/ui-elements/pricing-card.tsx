    ...
    >
      <Card
        className={`h-full transition-all duration-500 rounded-2xl overflow-hidden group relative flex flex-col ${
          popular 
            ? "shadow-2xl bg-card border border-teal-500/30 ring-1 ring-teal-500/20 z-10 scale-[1.02]" 
            : "shadow-sm hover:shadow-xl hover:-translate-y-1 bg-card border border-border/60"
        }`}
      >
        <CardContent className="p-8 flex flex-col h-full relative">
          ...
          <h3 className="text-xl font-bold mb-2 font-title text-foreground tracking-tight">{name}</h3>
          <div className="flex items-baseline gap-1 mb-4">
            <span className="text-4xl font-bold font-title text-foreground tracking-tighter">{price}</span>
